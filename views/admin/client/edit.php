...
<div class="form-container">
    ...
    <form id="editForm">
        <div class="form-group">
                <label>Nomp</label>
                <input type="text" name="nomp" value="<?= htmlspecialchars($item['nomp'] ?? '') ?>" class="form-control" required>
            </div>
            <div class="form-group">
                <label>Age P</label>
                <input type="text" name="age p" value="<?= htmlspecialchars($item['age p'] ?? '') ?>" class="form-control" required>
            </div>
...
document.getElementById('editForm').addEventListener('submit', async (e) => {
    ...
    const response = await fetch(`/api/client/${itemId}`, {
        method: 'PUT',
