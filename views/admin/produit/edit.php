...
<div class="form-container">
    ...
    <form id="editForm">
        <div class="form-group">
                <label>Prix</label>
                <input type="text" name="prix" value="<?= htmlspecialchars($item['prix'] ?? '') ?>" class="form-control" required>
            </div>
            <div class="form-group">
                <label>Nom</label>
                <input type="text" name="nom" value="<?= htmlspecialchars($item['nom'] ?? '') ?>" class="form-control" required>
            </div>
        
        <button type="submit" class="btn btn-primary">Update</button>
        <a href="/admin/produit" class="btn">Cancel</a>
    </form>
</div>
...
document.getElementById('editForm').addEventListener('submit', async (e) => {
    ...
    const response = await fetch(`/api/produit/${itemId}`, {
        method: 'PUT',
